...
class LikeButton extends StatefulWidget {
  const LikeButton({
    ...
    this.likeCountAnimationDuration = const Duration(milliseconds: 1),
    ...
    this.bubblesColor = const BubblesColor(
      ...
      dotLastColor: Colors.blue,
    ),
    this.circleColor = const CircleColor(start: Colors.white, end: Colors.black),
    this.onTap,
    this.countPostion,
    this.padding,
...
class LikeButtonState extends State<LikeButton> with TickerProviderStateMixin {
  ...
  Widget _createLikeCountWidget(int likeCount, bool isLiked, String? text) {
    return widget.countBuilder?.call(likeCount, isLiked, text) ?? Text('', style: TextStyle(color: Colors.grey));
  }
  ...
  onTap() async {
    ...
    imageCache.clear();
    imageCache.clear();
    _handleIsLikeChanged(widget.product?.is_liked != 1 ? true : false);
    Future.delayed(const Duration(milliseconds: 400), () {
      //  if (widget.product!.isLiked.toString() == '0') {
      // sound();
      vibrationMethod();

      function.setFavorite(widget.product!,false,widget.product?.is_liked != 1 ? true : false);

      // } else {
      //   for (int i = 0; i < favoritelist!.length; i++) {
      //     if (favoritelist![i].product_id == widget.product!.product_id) {
      //       function.setFavorite(widget.product);
      //     }
      //   }
      // }
    });
  ...
  _handleIsLikeChanged(bool isLiked) {
    if (_isLiked == null) {
      if (_likeCount != null) {
        _preLikeCount = _likeCount;
        _likeCount++;
