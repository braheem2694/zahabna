...
class _sectionsState extends State<sections> {
  ...
  Widget build(BuildContext context) {
    return Obx(() => widget.loading!.value
        ...
        :  globalController.homeDataList.value.products!
        .where((product) => product.product_section.toString() == globalController.homeDataList.value.productSections?[widget.index!].id.toString())
        .toList().isNotEmpty &&  globalController.homeDataList.value.products!
        .where((product) => product.product_section.toString() == globalController.homeDataList.value.productSections?[widget.index!].id.toString())
        .toList().first.cart_id != null
            ? Column(
                key: scaffoldKey,
                children: [
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        Text(
                          globalController.homeDataList.value.productSections![widget.index!].sectionName.toString(),
                        ...
                        CustomButton(
                          onTap: () {
                            var f = {
                              "section": [
                                globalController.homeDataList.value.productSections?[widget.index!].id.toString(),
                              ],
                            };
                            String jsonString = jsonEncode(f);

                            Get.toNamed(AppRoutes.View_All_Products, arguments: {
                              'title': globalController.homeDataList.value.productSections?[widget.index!].sectionName.toString(),
                              'id': globalController.homeDataList.value.productSections?[widget.index!].id.toString(),
                              'type': jsonString,
                            })!
                                .then((value) {
                  ...
                  globalController.homeDataList.value.productSections?[widget.index!].slider_type.toString() == '0' ? _buildProductRows( globalController.homeDataList.value.products!
                      .where((product) => product.product_section.toString() == globalController.homeDataList.value.productSections?[widget.index!].id.toString())
                      .toList()) : _buildListViewWidget( globalController.homeDataList.value.products!
                      .where((product) => product.product_section.toString() == globalController.homeDataList.value.productSections?[widget.index!].id.toString())
                      .toList()),
                ],
              )
      ...
      Widget row = Row(
        ...
        children: rowProducts.asMap().entries.map((entry) {
          ...
          return Expanded(
            child: Padding(
              padding: padding,
              child: ProductWidget(product: product, fromKey: "filter"),
            ),
          );
        }).toList(),
      );
